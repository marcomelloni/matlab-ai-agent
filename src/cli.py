...
def interactive(matlab: bool):
    ...
    agent.add_message("assistant", f"Generated MATLAB code:\n\n```matlab\n{matlab_code}\n```")
        ...
        if action == 1:
            ...
            if not agent.simulation_results.get("success", False):
                if click.confirm("\nAttempt to fix execution issues?"):
                    error = agent.simulation_results.get("error", "Unknown error")
                    click.echo("\nFixing code...")
            ...
            agent.add_message("assistant", f"Updated MATLAB code:\n\n```matlab\n{agent.matlab_code}\n```")
            ...
            # Save code to file
            filepath = click.prompt("Enter filename to save", default="matlab_simulation.m")
...
def execute(filepath: str):
    ...
    if agent.simulation_results.get("success"):
        ...
        if agent.simulation_results.get("figure"):
            click.echo(f"Figure saved to: {agent.simulation_results['figure']}")
    else:
...
def main():
    ...
    if not os.getenv("OPENAI_API_KEY"):
        ...
        click.echo("Create a .env file with your OpenAI API key or set it as an environment variable.")
        sys.exit(1)
